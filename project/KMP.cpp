...
int misMatchAllowed = 5; // 허용된 미스매치 횟수
int refDNANumber[10000] = { 0, }; // 복원된 MyDNA의 복원된 문자열의 자리. 복원됐으면 각 자리마다 있는 0을 1로 변경.
...
int main() {
	...
	string shortRead[100]; // 원래는 2,000만개, 알고리즘 테스트를 위해 500개로만 함
	string MyDNA2(10000, 'O');; // ShortRead로 복원한 데이터
	...
	for (index = 0; index < 100; index++) { // 원래는 2,000만번 비교
		start = 0;
		end = start + 50;
	...
	for (int i = 0; i < 10000; i++) {
		if (MyDNA2.substr(i, 1) != "O") {
			recover++;
			MyDNA2.substr(i, 1).replace(i, 1, refDNA.substr(i, 1));
		}
	}

	double recoverPercent = (double)recover / (double)10000;

	cout << "MyDNA 복원율 ::" << endl;
	cout << recoverPercent * 100 << "%" << endl;
	cout << endl;

	for (int i = 0; i < 10000; i++) { // 복원한 My DNA와 비교용 My DNA를 비교
		if (MyDNA.substr(i, 1) == MyDNA2.substr(i, 1)) {
			compare++;
		}
	}

	double comparePercent = (double)compare / (double)10000; // 일치율 계산
...
void kmpAlgorithm(string ref, string sr, string& md, int& start, int& end, int mM, int& correct) {
	...
	if (start < 9950 && end < 10000) { // start 지점이 (Reference의 길이 - ShortRead의 길이)를 넘어가버리면 비교가 불가능하므로 제약 조건을 걸어둔다.
		for (int i = 0; i < 50; i++) {
		...
		if (tmp != "") { // tmp 문자열의 접두사, 접미사를 이용하여 비교가 필요 없는 부분들은 건너뛴다.
			for (int i = 0; i < tmpLength; i++) {
				if (tmp.substr(i, 0) == tmp.substr(tmpLength - 1 - i, 1)) {
					count++;
				}
				else {
					break;
			...
			start += (tmpLength - count);
		}
		else if (tmp == "") { // tmp 문자열이 비면 그냥 한 칸 넘어감.
			start += count;
		}

		tmp.erase(0, tmpLength); // tmp 문자열을 비운다.
		end = start + 50;

		// 여기서부터는 main()에 있는 복원 과정과 동일하다.
		if (start < 9950 && end < 10000) {
			for (int a = start; a < end; a++) {
				if (refDNANumber[a] == 1) {
					count1++;
				}
		...
		else if (start >= 9950 || end >= 10000) { // 복원도 못하고 start가 (Reference의 길이 - ShortRead의 길이)를 넘어가버리면 다음 ShortRead로 넘어간다.
			correct = 1;
		}
	}
	else if (start >= 9950 || end >= 10000) { // start가 (Reference의 길이 - ShortRead의 길이)를 넘어가버리면 다음 ShortRead로 넘어간다.
		correct = 1;
	}
}