...
int misMatchAllowed = 10; // 허용된 미스매치 횟수
int refDNANumber[5000] = { 0, }; // 복원된 MyDNA의 복원된 문자열의 자리. 복원됐으면 각 자리마다 있는 0을 1로 변경.
...
int main() {
	...
	string MyDNA2(5000, 'O');; // ShortRead로 복원한 데이터
	...
	double recoverPercent = (double)recover / (double)5000;

	cout << "MyDNA 복원율 ::" << endl;
	cout << recoverPercent * 100 << "%" << endl;
	cout << endl;

	for (int i = 0; i < 5000; i++) { // 복원한 My DNA와 비교용 My DNA를 비교
		if (MyDNA.substr(i, 1) == MyDNA2.substr(i, 1)) {
			compare++;
		}
	}

	double comparePercent = (double)compare / (double)5000; // 일치율 계산
...
void kmpAlgorithm(string ref, string sr, string& md, int& start, int& end, int mM, int& correct) {
	...
	if (start < 4950 && end < 5000) { // start 지점이 (Reference의 길이 - ShortRead의 길이)를 넘어가버리면 비교가 불가능하므로 제약 조건을 걸어둔다.
		for (int i = 0; i < 50; i++) {
		...
		else if (start >= 4950 || end >= 5000) { // 복원도 못하고 start가 (Reference의 길이 - ShortRead의 길이)를 넘어가버리면 다음 ShortRead로 넘어간다.
			correct = 1;
		}
	}
	else if (start >= 4950 || end >= 5000) { // start가 (Reference의 길이 - ShortRead의 길이)를 넘어가버리면 다음 ShortRead로 넘어간다.
		correct = 1;
	}
}