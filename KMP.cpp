...
int misMatchAllowed = 3; // 허용된 미스매치 횟수
...
int main() {
	...
	string shortRead[500]; // 원래는 2,000만개, 알고리즘 테스트를 위해 500개로만 함
	string MyDNA2; // ShortRead로 복원한 데이터
	...
	for (index = 0; index < 500; index++) { // 원래는 2,000만번 비교
		start = 0;
		count1 = 0;
		end = start + 50;
		...
		do {
			...
			if (count1 == 0) { // count1이 0이면 비교 시작
				...
				if (misMatch <= misMatchAllowed) { // 미스매치가 3 이하이면
					MyDNA2.substr(start, 50) = shortRead[index];
					for (int a = start; a < end; a++) {
						refDNANumber[a] = 1;
